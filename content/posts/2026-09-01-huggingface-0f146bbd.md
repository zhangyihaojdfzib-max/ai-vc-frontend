...
title_original: 'Introducing @huggingface/kernels: 200+ WebGPU Kernels for Local AI'
date: '2026-09-01'
...
summary: '[翻译失败，原文如下]
  ...
  One of our biggest goals on the WebAI team at Hugging Face is to ma...'
categories:
- 未分类
tags: []
draft: false
translated_at: '2026-09-03T07:03:34.968900'
---
...
![Theai.onnx.Addrepository packages its manifest, correctness and benchmark cases, and WGSL shader templates together.](/images/posts/1f58d0b31593.png)

Behind the card, the repository contains the artifacts needed to understand and evaluate the implementation:

- manifest.jsonis the source of truth for the operation contract. It defines inputs, outputs, attributes, type constraints, and shape derivation rules.
- metadata.jsonrecords the kernel identifier, digests, and provenance.
- test.jsoncontains correctness cases, so an implementation can be checked against expected behavior.
- bench.jsoncontains benchmark and tuning cases that represent the workloads used to evaluate the kernel.
- *.wgsl.jinjafiles contain the parameterized WGSL implementations used to produce shaders for a particular request and device.
...
Theversion: 1option selects version 1 of the publishedkernel contract. It is separate from an ONNX opset, an operator'ssince_version, or a model revision. Keeping those concepts separate lets applications depend on a stable JavaScript-facing contract while kernel implementations evolve behind it.
...
![All 207 WebGPU kernels on the Hub'sKernels page, filtered by platform.](/images/posts/2f46cd0521a8.png)
...
> 本文由AI自动翻译，原文链接：[Introducing @huggingface/kernels: 200+ WebGPU Kernels for Local AI](https://huggingface.co/blog/webgpu-kernels)
> 
