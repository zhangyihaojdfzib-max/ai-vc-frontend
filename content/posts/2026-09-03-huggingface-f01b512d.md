...
title_original: Give Your Coding Agents a Memory You Own
date: '2026-09-03'
...
tags: []
draft: false
translated_at: '2026-09-04T07:07:17.054097'
---
...
leave behind a dense account of not justwhatchanged, butwhy.

While the diagnosis is correct, traces are only potential memory. The session logs of
an agent are still just an archive. You cannotgrepyour way to“why did we move
...
That is whatfunesprovides. It is a durable
...
and embedding and reranking happenon your machine. Install it:
...
That oneaddcommand builds the first index, gives the agentrecallandgettools, and installs the automation that indexes each completed turn. Indexing is
...
the agent can reach forrecallitself. You do not need to remember the old session or
paste its context into the new one.

![A coding agent reaches for funes on its own, recalls an earlier decision, and grounds its answer in the retrieved session](/images/posts/db4aaded539a.gif)
...
recallreturns the original text, not a summary, and shows exactly where it came from
(the agent, timestamp, session, and turn). Each result includes agetcommand that
...
a localLancedataset. A query combines vector and BM25 search, fuses their
...
same shape.recallspans their histories, and every hit says which agent produced
it.
...
- recallis local by default:No account or Hub repository is required. A hosted
...
Theagent as a strangerproblem is already solved on one machine. But memory gets
...
looks like a secret. The scanner behind this is documented inSECURITY.md, including
...
recallis shaped for agents. When you want to put a question to a memory yourself,
useask. It reads your local memory by default:
...
Or point it at a shared memory. We published amemoryof funes development,
...
![Asking the published funes memory why it is append-only; funes retrieves the relevant sessions and a coding agent answers from them](/images/posts/5b72e0e13944.gif)

funes askis the read-only, one-question sibling offunes add. It recalls the
...
![Claude Code chooses an embedding model, then Codex recalls that decision in a separate session](/images/posts/2b6496bc3cdb.gif)
...
release, naming them on the push. Think of it as a searchableCLAUDE.mdthat holds
...
anddiscoverable on the Hub. The Hub
...
![Weighted tokens per successful task for five channels across two tasks, with recall the shortest bar on both](/images/posts/a9a576dcf258.jpg)
...
Your agents already wrote the record. funes lives atgithub.com/huggingface/funes, one command away
...
run locally, onLance's append-only datasets with
...
funes is open source too.Open an issuefor anything from an install snag to a recall that missed, or an agent you'd like
supported.

---

> 本文由AI自动翻译，原文链接：[Give Your Coding Agents a Memory You Own](https://huggingface.co/blog/funes)
> 
