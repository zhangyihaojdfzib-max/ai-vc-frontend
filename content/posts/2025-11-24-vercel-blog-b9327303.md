...
author: ''
summary: 本文详细介绍了Vercel团队开发其首款移动应用v0 iOS版的技术历程。作为一家专注于Web的公司，他们为了打造能与苹果原生应用媲美的体验，尝试了多种技术方案，最终选择React
  Native与Expo。文章重点解析了如何构建流畅的AI聊天界面，包括消息动画、键盘控制、浮动输入框等核心功能，并分享了在实现过程中解决的技术挑战与创新模式。
categories:
- AI产品
tags:
- React Native
...
- 用户体验
draft: false
translated_at: '2026-01-12T04:56:59.593895'
---

我们最近发布了 iOS 版 `v0`，这是 Vercel 的首款移动应用。作为一家专注于 Web 的公司，构建原生应用对我们来说是一个全新的领域。

我们的目标是打造一款配得上苹果设计奖的应用，并且我们对实现这一目标的最佳技术栈持开放态度。为此，我们在公开测试版之前构建了数十个产品迭代版本。我们尝试了截然不同的技术栈和 UI 模式。

我们从那些体现 iPhone 设计语言的应用中汲取灵感，例如 Apple Notes 和 iMessage。`v0` 必须在您的主屏幕上与这些优秀应用并肩而立。
...
当您离开电脑时，可能有一个想要立即付诸实践的快速想法。我们的目标是让您无需切换上下文，就能将那个想法转化为切实可见的东西。iOS 版 `v0` 是您 Notes 应用的下一代产品，您的想法将在后台被构建出来。

我们并非要构建一个功能与网站完全对等的移动 IDE。相反，我们希望打造一个简单、愉悦的体验，让您能在移动中使用 AI 来创造事物。这一体验的核心就是聊天。
...
*   新的用户消息滚动到屏幕顶部
*   助手消息在流式传输时以交错过渡效果淡入
*   输入框使用 Liquid Glass 效果并浮动在可滚动内容之上
*   打开现有聊天时，初始位置滚动到底部
*   键盘处理感觉自然
*   文本输入框允许您粘贴图片和文件
*   文本输入框支持平移手势来聚焦和失焦
...
虽然移动应用中已经出现了一些用于 AI 聊天的 UI 模式，但在移动端进行 AI 代码生成方面，还没有一套等效的模式。

我们在现有的 React Native 应用中未曾见过这些功能，因此我们发现自己需要即时发明模式。为了让每个功能都达到我们的标准，我们投入了大量的工作、测试和协调。

## 构建可组合的聊天

为了满足我们的要求，我们将聊天代码结构设计为**可组合的**，以每个功能为基础。
...
以下部分将分解每个钩子，以展示它们如何协同工作。

## 发送您的第一条消息

当您在 `v0` 上发送消息时，消息气泡会平滑地淡入并滑动到顶部。在用户消息动画完成后，助手消息会立即淡入。

当用户发送消息时，我们设置一个 Reanimated 共享值来指示动画应该开始。共享值允许我们更新状态而不会触发重新渲染。
...
由于我们的状态在 Reanimated 中被跟踪，我们现在可以动画化我们的 `UserMessage`。
...
请注意，`UserMessageContent` 被包裹在一个 `Animated.View` 中，该视图接收来自 `useFirstMessageAnimation` 的属性。
...
基于消息高度、窗口高度和当前键盘高度，`getAnimatedValues` 为 `translateY` 和 `progress` 构建了缓动函数、`start` 和 `end` 状态。生成的共享值分别作为 `transform` 和 `opacity` 传递给 `useAnimatedStyle`。

就这样，我们使用 Reanimated 实现了第一条消息的淡入效果。一旦动画完成，我们就可以淡入第一条助手消息回复了。
...
如果你打开一个已有一条用户消息和一条助手消息的现有聊天，它会再次执行动画吗？不会，因为这里的动画仅在 `isMessageSendAnimating` 为 `true` 时应用，而该值在 `onSubmit` 时设置，并在你切换聊天时被清除。
...
动态高度是虚拟化列表中的常见挑战。频繁更新的空白区域大小将这一挑战提升到了一个新的高度。我们的列表项具有动态、未知且频繁更新的高度，并且我们需要它们浮动到顶部。

对于足够长的助手消息，空白区域大小可能为零，这引入了一系列新的边界情况。
...
我们尝试了许多不同的方法来实现空白区域大小。我们尝试在 `ScrollView` 底部放置一个具有高度的 `View`，在 `ScrollView` 本身设置底部内边距，对可滚动内容使用 `translateY`，以及在最后一条系统消息上设置最小高度。所有这些方法最终都产生了奇怪的副作用和较差的性能，通常是由于需要与 Yoga 进行布局计算。

我们最终确定了一个解决方案，该方案使用 `ScrollView` 上的 `contentInset` 属性来处理空白区域大小，而不会产生抖动。`contentInset` 直接映射到 UIKit 中 `UIScrollView` 的原生属性。
...
构建良好的聊天体验，关键在于优雅的键盘处理。在 React Native 中实现原生的感觉在这一领域是繁琐且具有挑战性的。当 `v0` iOS 处于公开测试阶段时，苹果发布了 iOS 26。每次新的 iOS 测试版发布，我们的聊天功能似乎都会完全崩溃。每个 iOS 版本都变成了一场猫鼠游戏，需要重现微小的差异和抖动。

幸运的是，`react-native-keyboard-controller` 的维护者 Kiryl 帮助我们解决了这些问题，他经常在苹果发布新测试版的 24 小时内更新库。

### 构建 `useKeyboardAwareMessageList`

我们使用了 React Native Keyboard Controller 提供的许多钩子，来构建我们专为 `v0` 聊天定制的键盘管理系统。

`useKeyboardAwareMessageList` 是我们自定义的 React 钩子，负责处理我们所有的键盘处理逻辑。我们将其与聊天列表一起渲染，它抽象了所有我们需要让键盘感觉正确的细节。
...
`useKeyboardAwareMessageList` 还处理了 iOS 中的许多奇怪行为。例如，如果你在键盘打开时将应用切换到后台，然后重新聚焦该应用，iOS 会莫名其妙地触发三次键盘 `onEnd` 事件。由于我们在事件触发时依赖于命令式行为，我们想出了一些技巧来去重重复事件并跟踪应用状态变化。
...
6.  如果你滚动到聊天末尾且空白区域大于零，但当键盘打开时它应为零，则将内容上移，使其位于键盘上方
...
如果你滚动到聊天末尾且空白区域大于零，但当键盘打开时它应为零，则将内容上移，使其位于键盘上方

没有单一的技巧能让这一切正常工作。我们花费了数十个小时使用该应用，注意到不完美之处，追踪问题，并重写逻辑，直到感觉正确。
...
由于动态消息高度和空白区域的复杂组合，我们必须多次调用 `scrollToEnd`。如果不这样做，我们的列表要么无法正确滚动，要么滚动得太晚。一旦内容滚动完毕，我们调用 `hasScrolledToEnd.set(true)` 来淡入聊天界面。
...
我们使用 `@callstack/liquid-glass` 来添加交互式液态玻璃效果。通过用 `LiquidGlassContainerView` 包裹玻璃视图，我们自动获得了视图变形效果。
...
添加液态玻璃效果后，下一步是使其浮动在聊天内容上方。

为了使输入框浮动在可滚动内容之上，我们采取了以下步骤：

1.  为输入框添加 `position: absolute; bottom: 0`
2.  将输入框包裹在 `react-native-keyboard-controller` 的 `KeyboardStickyView` 中
3.  同步测量输入框，并使用共享值将其高度存储在上下文中
4.  将 `composerHeight.get()` 添加到我们 ScrollView 的原生 `contentInset.bottom` 属性中

为输入框添加 `position: absolute; bottom: 0`

将输入框包裹在 `react-native-keyboard-controller` 的 `KeyboardStickyView` 中

同步测量输入框，并使用共享值将其高度存储在上下文中
...
在下面的视频中，你可以看到两种情况。在视频开始时，由于聊天已滚动到底部，内容会随着新行的输入而上移。但是，在聊天中向上滚动后，输入新行将不会移动内容。
...
`useScrollWhenComposerSizeUpdates` 让我们有条件地模拟非绝对定位视图的体验。
...
正如我们在前面的代码中看到的，不幸的是，我们依赖了许多 `setTimeout` 和 `requestAnimationFrame` 调用来执行 `scrollToEnd`。这段代码无疑会引起质疑，但这是我们设法让滚动到底部正常工作的唯一方法。我们正在积极与 LegendList 的维护者 Jay 合作，以构建一种更可靠的方法。

### 链接到标题让它感觉原生

React Native 内置的 `TextInput` 在原生聊天应用中显得格格不入。

默认情况下，当你设置 `multiline={true}` 时，`TextInput` 会显示难看的滚动指示器，这与大多数聊天应用不一致。在输入框上向上或向下滑动会使其内部内容弹跳，即使你还没有输入任何文本。此外，输入框不支持交互式键盘关闭。
...
虽然在 React Native 更新中维护一个补丁并不理想，但这是我们找到的最实用的解决方案。我们更希望有一个官方的 API 来扩展原生视图而无需打补丁，如果有社区兴趣，我们计划将这个补丁贡献给 React Native 核心。
...
由于在原生代码中扩展现有的 `TextInput` 很困难，我们使用了一个 `TextInputWrapper` 组件来包装 `TextInput`，并在 Swift 中遍历其 `subviews`。有关创建原生包装器组件的更深入示例，你可以观看我 2024 年的演讲《不要害怕构建原生库》。
...
只要一个元素被这些组件之一包装，其子元素就会以交错的动画平滑地淡入。
...
`useIsAnimatedInPool` 是一个位于 React 外部的自定义状态管理器，它允许有限数量的有序元素同时被渲染。元素在挂载时请求加入池，而 `isActive` 指示它们是否应渲染一个动画节点。

在 `onFadedIn` 回调触发后，我们将元素从池中移除，直接渲染其子元素而无需动画包装器。这有助于我们限制同时处于活动状态的动画节点数量。

最后，`FadeIn` 会渲染一个交错动画，元素之间的延迟为 32 毫秒。交错动画按计划运行，每次批量动画处理 2 个项目。当交错项目的队列长度超过 10 时，我们会根据队列的大小增加批量处理的项目数量。
...
`TextFadeInStaggeredIfStreaming` 使用了类似的策略。我们首先将单词分割成单独的文本节点，然后为文本元素创建一个限制为 4 的唯一池。这确保一次最多只有 4 个单词会淡入。
...
我们使用这种方法遇到的一个问题是，它严重依赖于在挂载时触发动画。因此，如果你发送一条消息，转到另一个聊天，然后在消息发送完成之前回到原始聊天，它会重新挂载并再次播放动画。

为了缓解这个问题，我们实现了一个系统，用于跟踪你在不同聊天中已经看到过动画的内容。该实现在树中消息的顶部附近使用了 `DisableFadeProvider`。我们在根淡入组件中使用它，以便在需要时避免影响池。
...
在成熟的 Next.js 应用和新的移动应用之间共享后端 API 路由带来了挑战。`v0` Web 应用由 React 服务器组件和服务器操作驱动，而移动应用的功能更像一个单页 React 应用。

为了解决这个问题，我们使用一个手工打造的后端框架构建了一个 API 层。我们的框架通过要求使用 `Zod` 指定输入和输出类型来强制执行运行时类型安全。

定义路由后，我们基于每个路由的 Zod 类型生成一个 `openapi.json` 文件。移动应用使用 `Hey API` 来消费 OpenAPI 规范，它会生成与 `Tanstack Query` 一起使用的辅助函数。
...
这项工作促成了 `v0` 平台 API 的开发。我们希望为我们自己的原生客户端构建理想的 API，并最终决定向所有人开放该 API。得益于这种方法，`v0` 移动应用使用了与 `v0` 平台 API 客户相同的路由和逻辑。

在每次提交时，我们都会运行测试，以确保对 OpenAPI 规范的更改与移动应用兼容。

未来，我们希望通过在平台 API 之上使用类型级别的 RPC 包装器来完全消除代码生成步骤。

## 样式

`v0` 使用 `react-native-unistyles` 进行样式设置和主题管理。我在 React Native 方面的经验告诉我，要谨慎对待在渲染过程中完成的任何工作。与我们评估过的其他样式库不同，Unistyles 提供了全面的主题功能，而无需重新渲染组件或访问 React Context。

## 原生菜单

除了用于主题和样式的 Unistyles 之外，我们没有使用基于 JS 的组件库。相反，我们尽可能依赖原生元素。

对于菜单，我们使用了 `Zeego`，它在底层依赖 `react-native-ios-context-menu` 来渲染原生 `UIMenu`。当你使用 Xcode 26 构建时，Zeego 会自动渲染 Liquid Glass 菜单。

## 原生警告

在 iOS 26 上，React Native 应用的 `Alert` 弹窗会渲染到屏幕外。我们在自己的应用以及许多流行的 React Native 应用中重现了这个问题。我们在本地进行了修补，并与 Callstack 和 Meta 的开发人员合作，在 React Native 中上游修复了这个问题。
...
首先，当向下拖动表单时，它会在正确关闭之前暂时冻结在原位。为了解决这个问题，我们在本地修补了 React Native。我们与 Callstack 合作，将我们的补丁上游到 React Native 中，现在它已在 0.82 版本中生效。

### 修复 Yoga 闪烁问题

如果你将一个带有 `flex: 1` 的 `View` 放在一个有背景色的模态框内，然后上下拖动模态框，视图的底部会剧烈闪烁。

为了解决这个问题，我们在本地修补了 React Native，以支持 Yoga 中模态框的同步更新。我们与来自 Callstack、Expo 和 Meta 的开发人员合作，将这一更改上游到 React Native 核心中。现在它已在 React Native 0.82 版本中生效。

## 展望未来

在使用 React Native 和 Expo 构建了我们的第一个应用之后，我们不再回头。如果你还没有尝试过 iOS 版的 `v0`，请下载它，并通过 App Store 评价告诉我们你的想法。

我们正在招聘开发人员加入 Vercel 移动团队。如果这类工作让你感到兴奋，我们很乐意收到你的来信。
